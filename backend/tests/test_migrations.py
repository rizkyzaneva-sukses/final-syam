...
def test_fresh_database_migrates_to_head():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        with closing(sqlite3.connect(database)) as db:
            assert db.execute("SELECT version_num FROM alembic_version").fetchone()[0] == "0006_full_cost_review"
            names = {row[0] for row in db.execute("SELECT name FROM sqlite_master WHERE type='table'")}
            assert {"orders", "quotations", "bom_items", "material_consumptions", "production_cost_entries", "cost_reviews", "qc_records"} <= names
