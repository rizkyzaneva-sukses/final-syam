...
import {slaBadge,handoffStatus,taskId,poHandoff} from '../queue';
...
/* Status handoff per-order untuk satu baris PO (blueprint REF-DEBY poin 3-4):
   kontraknya ada di `../queue.js` (poHandoff) supaya kosakatanya sama dengan
   antrean PO Inbox, Draft Order dan Quotation — halaman tidak menyalin label. */
...
export default function POInboxPage(){
  ...
  return <div className="page">
    ...
    <div className="table-scroll"><table><thead><tr>
      <th>Task ID</th><th>PO ID</th><th>Buyer ID</th><th>Nomor / Tanggal PO</th><th>Article ID</th><th>Dokumen buyer</th><th>Order type</th>
      <th>Currency / Terms</th><th>Jumlah article</th><th>Kelengkapan</th><th>Missing items</th>
      <th>Reason perbaikan</th><th>Received at</th><th>Source / evidence</th><th>Updated at</th><th>Next action</th><th>Due / SLA internal</th><th>Review Cecep</th><th>Handoff</th><th>Aksi</th>
    </tr></thead><tbody>
      {shown.map(row=>{
        ...
        /* Ramah ke server lama: task_id dipakai kalau endpoint sudah mengirimnya,
           kalau tidak dirakit dari nomor PO (rancangan batch 2). */
        const tid=row.task_id||taskId('PO',row.po_number||row.id);
        const sla=slaBadge(queue.due),handoff=poHandoff(row);
        return <tr key={row.id}>
          ...
          <td>{queue.action}</td>
          <td>{queue.due?<><span className={'badge '+sla.tone}>{sla.label}</span><br/><small>Due {queue.due}</small></>:<span className="badge gray">Tanpa due date</span>}</td>
      ...
      {!shown.length&&<tr><td colSpan={20} className="empty">Belum ada PO terdaftar.</td></tr>}
    </tbody></table></div>
    {/* Pintu masuk 'PO Masuk' dari Draft Order (blueprint poin 3 & 4): order yang
    ...
    {canPrepare&&<section className="panel">
      ...
      <div className="table-scroll"><table><thead><tr>
        <th>Task ID</th><th>Draft Order ID</th><th>Buyer ID</th><th>Draft Order type</th><th>Article ID</th><th>Jumlah article</th><th>Kelengkapan draft</th><th>Deadline buyer</th><th>SLA internal</th><th>Next action</th><th>Handoff</th><th>Aksi</th>
      </tr></thead><tbody>
        {shownDrafts.map(order=>{
          ...
          const sla=slaBadge(order.buyer_deadline);
          return <tr key={order.id}>
            <td><b>{taskId('PO',order.order_id)}</b></td>
            ...
            <td>{order.buyer_deadline?<span className={'badge '+sla.tone}>{sla.label}</span>:<span className="badge gray">Tanpa due date</span>}</td>
            <td>Buat PO intake, unggah dokumen buyer, lalu Cek Kelengkapan dan kirim ke Cecep</td>
            <td><span className="badge blue">CMO_SUPPORT (Deby)</span><br/><small>Berikutnya: CMO_MANAGER (Cecep) setelah PO intake dikirim</small></td>
        ...
        {!shownDrafts.length&&<tr><td colSpan={12} className="empty">Tidak ada order draft tanpa PO{q?` untuk pencarian “${q}”`:''}. Semua order yang ada sudah punya PO intake tertaut.</td></tr>}
      </tbody></table></div>
    </section>}
