...
export default function SPKPage(){
  ...
  async function act(spk,action){
    if(action==='release'&&!confirm(`Release SPK ${spk.spk_no} ke produksi?`))return;
  ...
  return <div className="page">
    ...
    <div className="table-scroll"><table><thead><tr><th>No. SPK</th><th>Versi</th><th>Order</th><th>Status</th><th>Catatan</th><th>Aksi</th></tr></thead>
      <tbody>{filtered.map(s=><tr key={s.id}>
        ...
        <td className="td-action">
          ...
          {manager&&s.status==='PRINTED'&&<button className="btn primary" disabled={busy} onClick={()=>act(s,'release')}><Check size={14}/> Release</button>}
      ...
      </tr>)}{filtered.length===0&&<tr><td colSpan={6} className="empty">Tidak ada SPK</td></tr>}</tbody></table></div>
